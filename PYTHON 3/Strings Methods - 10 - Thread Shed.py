...
for i in daily_transactions_split:
    cleaned_transaction = []
    for j in i:
        cleaned_transaction.append(j.strip())
...
for i in thread_sold:
  if "&" in i:
    for j in i.split("&"):
      thread_sold_split.append(j.strip())
  else:
    thread_sold_split.append(i.strip())
...
colors = ["white", "black", "purple", "green", "blue", "yellow", "red"]
...
for i in colors:  
  print("We sold {} threads of {} today.".format(color_count(i), i))
print("--------------------------------------------------")
